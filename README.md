...
This function, like the brute force, also creates or overrides a file called, in this case, simple_answer.dat, which contains the following contents:
...
More important than the number of clauses, however, is the number of variables, which is this case is 50 (x1, x2, ..., x49, x50). Selecting the heuristic  method, as we would expect, takes a little under 50 iterations, and produces the following output:
...
Note also that, in order to keep vertexes distrinct from each other, it was necessary to generate a new number for each one, representing the its individual identity, as opposed to its x-value. In this case, the mappings can we seen at the bottom of the output file, which contrains the following contents:
...
Note that the example 3SAT formula given exactly matches the input found in simple2.dat.
Note further that the output found in simple2_to_VC.dat exactly matches the input found on the site.
