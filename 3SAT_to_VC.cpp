...
// Compile with: g++ -std=c++11 -Wall -o 3sat_to_VC 3SAT_to_VC.cpp
#include <iostream>
#include <fstream>
#include <string>
#include <sstream>
...
void createOutputFile(string inputFileName, string mainOutput, string mappingOutput)
{
    ...
    string outputFileName = inputFileName.substr(0, inputFileName.find_last_of(".")) + "_to_VC.dat";
...
int main(int argc, char *argv[])
{
    ...
    // PART 2: Write the output file for Vertex Cover.
    ...
    //    2. We will add an extra vertex for the positive and negative values of each x-variable.
    //          The positive will be connected to the negative, and vice versa.
    //          For each of these values, create a new connection to (and from) all other instances
    //          of that variable found in the triangles (as found in the mappings array).

    string mainOutput = "";
    // This is the array used to keep track of the mappings.
    //    For instance, if the value at index 3 is 8, then vertex 3 is actually x8.
    int mappings[clauses.size() * 3 + 1 + (num_vars * 2)];
    ...
    //    then we must find all other instances of x8.
    ...
    mainOutput += to_string((clauses.size() * 3) + (num_vars * 2)); // The first line of the output file will be the number of vertices
    mainOutput += "\n";
    int count = 1;
    for (int i = 0; i < clauses.size(); i++)
    {
    ...
    // Now we will add a new vertex for the positive and negative values of each x-variable.
    for (int i = 1; i <= num_vars; i++)
    {
        // Create the main output
        // Positive vertex to negative vertex.
        mainOutput += to_string(count); // positive vertex
        mainOutput += " ";
        mainOutput += to_string(count + 1); // negative vertex
        mainOutput += " 1\n";               // Edge weight

        //     Reverse of that edge (because graph is undirected)
        mainOutput += to_string(count + 1); // negative vertex
        mainOutput += " ";
        mainOutput += to_string(count); // positive vertex
        mainOutput += " 1\n";           // Edge weight
        ...
        // Create the mappings
        mappings[count] = i;
        mappings[count + 1] = -1 * i;

        // Add connections to all other instances of the positive variable.
        vector<int> &vector_of_pos_matches = map_of_vector[i];
        for (int j = 0; j < vector_of_pos_matches.size(); j++)
        {
            mainOutput += to_string(count);
            mainOutput += " ";
            mainOutput += to_string(vector_of_pos_matches[j]);
            mainOutput += " 1\n";
        ...
        // Add connections to all other instances of the negative variable.
        vector<int> &vector_of_neg_matches = map_of_vector[-1 * i];
        for (int j = 0; j < vector_of_neg_matches.size(); j++)
        {
            mainOutput += to_string(count + 1);
            mainOutput += " ";
            mainOutput += to_string(vector_of_neg_matches[j]);
            mainOutput += " 1\n";
    ...
    for (int i = 1; i < clauses.size() * 3 + 1 + (num_vars * 2); i++)
    {
